...
contract NftGame {
    ...
    ////////////////////// New registry
    function register(uint256 _nft, uint256 _wormLvl, uint256 _gameCost, bytes32 _location) external payable gameHasStarted notPaused noContract returns (bool) {
        ...
        if (msg.value < _gameCost) revert  NftGame__IncorrectAmount();
    ...
    function nextRound(uint256 _nft, uint256 _wormLvl, uint256 _gameCost, bytes32 _location) external payable gameHasStarted notPaused noContract returns (bool) {
    ...
    function reviveMingle(uint256 _nft) private gameHasStarted notPaused returns (bool) {
        ...
        uint256 yourChanceToRevive = revive(users[_nft].wormLvl) + 1;
        uint256 decition = revivechoices();
    ...
    ////////////////////// Trigger random choice to compare with "revive" function
    function revivechoices() private view returns (uint256) {
        uint256 num = 100;
        return
            uint256(
                keccak256(
    ...
    ////////////////////// Trigger random revive choice to compare with "revivechoices" function
    function revive(uint256 numR) private view returns (uint256) {
        uint256 num = 100;
        return
            uint256(
                keccak256(
    ...
    function choice(uint256 _nft, bytes32 _location) external gameHasStarted notPaused noContract returns (bool){
        if (ERC721(nftAddressPlayer).ownerOf(_nft) != msg.sender) revert NftGame__MingleNotOwned();
        if (users[_nft].stage == 6) revert NftGame__AlreadyASurvivor();
        
        if (randomchoices() + 1 > dead() + 1) {
    ...
    ////////////////////// Trigger random chance to die in battle
    function dead() private view returns (uint256) {
        uint256 num = 100;
        return
            uint256(
                keccak256(
    ...
    function escape(uint256 _nft) private gameHasStarted notPaused returns (bool) {
        mingles.push(_nft);
